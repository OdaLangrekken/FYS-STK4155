...
for i in range(1, pol_degree+1):
    ...
    mse_bs_test, mse_bs_train = bootstrap(lr_bs, X_train, z_train, X_test, z_test, num_iterations, sample_size=1)
    mses_bs_train.append(mse_bs_train)
    mses_bs_test.append(mse_bs_test)

    lr_bias_variance = LinearModel()
    # Decompose bias and variance using bootstrap
    bias, variance, mse_bias_var =  bias_variance_decomp(lr_bias_variance, X_train, z_train, X_test, z_test, boostrap_iterations=num_iterations)
    biases.append(bias)
    variances.append(variance)
    mses_bias_vars.append(mse_bias_var)

    # Find MSE using cross bal
    mse_test_cv, mse_train_cv = cross_validation(lr, X, z, 5)
...
plot_bias_variance(biases, variances, mses_bias_vars, pol_degree, save_plot=save_fig, save_title='bootstrap_biasVar_n=' + str(data_size) + ', sigma=' + str(error_std) + ', iterations=' + str(num_iterations))
plt.show()
plt.clf()

plot_mse_per_poldegree([mses_train_cv, mses_test_cv, mses_bs_train, mses_bs_test], ['CV train', 'CV test', 'Bootstrap train', 'Bootstrap test'], pol_degree=pol_degree, save_plot = save_fig, save_title = 'bootstrap_crossval_n=' + str(data_size) + ', sigma=' + str(error_std) + ', iterations=' + str(num_iterations))
plt.show()