...
for i in range(1, pol_degree+1):
    ...
    mse_bs_test, mse_bs_train = bootstrap(X, z, 50, sample_size=1)
    mses_bs_train.append(mse_bs_train)
    mses_bs_test.append(mse_bs_test)

# Plot MSE
plot_mse_per_poldegree([mses_train, mses_test, mses_bs_train, mses_bs_test], ['OLS_train', 'OLS_test', 'Bootstrap train', 'Bootstrap test'], pol_degree=pol_degree, save_plot = save_fig, save_title = 'OLS_MSE_n=' + str(data_size) + ', sigma=' + str(error_std))
